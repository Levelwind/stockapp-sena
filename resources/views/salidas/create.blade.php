...
@section('header')
<h1 class="text-2xl font-semibold text-white tracking-tight">Registrar Salida de Productos (Venta)</h1>
@endsection
@section('content')

<form method="POST" action="{{ route('salidas.store') }}" class="space-y-6">
    @csrf
    
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-white/[0.04] border border-white/10 rounded-xl">
        <div>
            <label class="field-label">Cliente:</label>
            <select name="cliente_id" required class="field-input">
    ...
    <div>
        <div class="flex items-center justify-between mb-2">
            <h3 class="text-lg font-semibold text-white">Productos a vender</h2>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-white/[0.04] text-xs text-slate-400 uppercase font-medium">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-left w-48">Precio Venta ($)</th>
    ...
    <div class="flex items-center gap-4 pt-4 border-t border-white/10">
        ...
        <a href="{{ route('salidas.index') }}" class="text-sm font-medium text-slate-400 hover:text-white transition underline underline-offset-2">Cancelar</a>
    </div>
</form>
...
function agregarFila() {
    ...
    const opciones = productos.map(p =>
        \`<option value='\${p.id}'>\${p.nombre} (\${p.referencia}) — Stock: \${p.stock_actual} | Precio sugerido: $\${p.precio_venta}</option>\`
    ).join('');

    const fila = \`
        <tr id='fila-\${i}' class="hover:bg-white/[0.02] transition">
            <td class="px-6 py-3">
                <select name='productos[\${i}][id]' required class="block w-full rounded-md border-white/10 py-1.5 text-sm focus:border-indigo-500 focus:ring-indigo-500" onchange="actualizarPrecio(this, \${i})">
                    <option value=''>-- Seleccione --</option>
                    \${opciones}
                </select>
            </td>
            ...
            <td class="px-6 py-3">
                <div class="relative rounded-md shadow-sm">
                    ...
                    <input type='number' name='productos[\${i}][precio]' id='precio-\${i}' min='0' step='0.01' required class="block w-full rounded-md border-white/10 pl-7 py-1.5 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                </div>
            </td>
...
function actualizarPrecio(selectElem, index) {
    const prodId = selectElem.value;
    const productoInfo = productos.find(p => p.id == prodId);
    if(productoInfo) {
        document.getElementById('precio-' + index).value = productoInfo.precio_venta;
    } else {
